...
GarmentsReceiptNoteModel.prototype = {
    ...
    delete: function (id, callback) {
        DBCON.query(`delete from ${TABLE_NAME} where id = ?`, id, (err, result) => {
            if (err) {
                ...
                DBCON.query(`delete from garments_receipt_note_inventory where vou_id = ?`, id, (err, result1) => {
                    if (err) {
                        callback(err)
                    } else {
    ...
    getGarmentsReceiptNotePrint: (id, callback) => {
        if (issetNotEmpty(id)) {
            DBCON.query(`select * from garments_receipt_note where garments_receipt_note.id = ${id}`, (err, result) => {
                if (err) {
                    callback(err)
                } else {
                    if (result.length > 0) {
                        ...
                        callback("Receipt Note not found!")
                    }
                }
            })
        } else {
