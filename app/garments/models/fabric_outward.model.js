...
FabricOutwardModel.prototype = {
    ...
    getAll : function(callback){
        DBCON.query(`select fabric_outward.id,fabric_outward.vouno, DATE_FORMAT(fabric_outward.vou_date, '%d-%m-%Y') as vou_date,ledger.ledger from ${TABLE_NAME} left join ledger on ledger.id=fabric_outward.ledger_id order by ${TABLE_NAME}.id desc`, function(err, result){
    ...
    delete : function(id, callback){
        DBCON.query(`delete from ${TABLE_NAME} where id = ?`, id, (err,result) => {
            if(err)
            {
                ...
                DBCON.query(`delete from fabric_outward_inventory where vou_id = ?`, id, (err,result1) => {
                    if(err)
                    {
                        callback(err)
                    }
