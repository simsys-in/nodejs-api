...
JobworkInvoiceModel.prototype = {
    ...
    getAll: function (callback) {
        DBCON.query(`select jobwork_invoice.id,jobwork_invoice.inventory_qty_total, DATE_FORMAT(jobwork_invoice.vou_date, '%d-%m-%Y') as vou_date,jobwork_invoice.vouno, ledger.ledger from ${TABLE_NAME} left join ledger on ledger.id=jobwork_invoice.ledger_id  order by ${TABLE_NAME}.id desc`, function (err, result) {
                ...
                if (err) {
                    ...
                    DBCON.query(`delete from jobwork_invoice_inventory where vou_id = ?`, body.id, (err, deletedData) => {
                        if (err) {
                            ...
                            if (body.jobwork_invoice_inventory.length > 0) {
    ...
    delete: function (id, callback) {
        DBCON.query(`delete from jobwork_invoice_inventory where vou_id = ?`, id, (err, result) => {
            if (err) {
                callback(err)
            } else {
                if (err) {
                    ...
                    DBCON.query(`delete from jobwork_invoice where id = ?`, id, (err, result1) => {
