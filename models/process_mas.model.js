...
ProcessModel.prototype = {
    ...
    checkAndSaveOrUpdate: function (body, callback) {
        ...
        if (issetNotEmpty(body.id)) {
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where id != ? and process = ?`, [body.id, body.process], (err, count) => {
                if (err) {
                    callback(err)
                } else {
                    if (count[0].c > 0) {
                        ...
                        DBCON.query(`update ${TABLE_NAME} set ? where id = ?`, [body, body.id], (err, result) => {
                            if (err) {
                                ...
                                callback(false, result, "Process Updated Successfully")
                            }
                        })
                    }
                }
            ...
            // body.created_at = new Date();
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where process = ?`, [body.process], (err, count) => {
                if (err) {
                    callback(err)
                } else {
                    // console.log("DB Query Success")
                    if (count[0].c > 0) {
                        callback("Process Name Already Found!")
                    } else {
                        DBCON.query(`insert into ${TABLE_NAME} set ?`, body, (err, result) => {
                            if (err) {
                                callback(err)
                            } else {
                                callback(false, result, "Process Saved Successfully!")
                            }
                        })
                    }
                }
