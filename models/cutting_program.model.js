...
CuttingProgramModel.prototype = {
    find: function (match = null, callback) {
        ...
        pool.query(sql, match, function (err, result) {
            ...
            if (result.length) {
                result[0].fabrics = [];
                pool.query(`select * from cutting_program_inventory where vou_id = ?`, match,(err, inventory) => {
                    if(err)
                    {
                        console.log(err);
                        callback(err);
    ...
    getAll : function(callback){
        pool.query(`select ${TABLE_NAME}.id, ${TABLE_NAME}.lotno, ${TABLE_NAME}.vou_date, order_program.order_no  from ${TABLE_NAME} left join order_program on order_program.id = ${TABLE_NAME}.order_id order by ${TABLE_NAME}.id desc`, function(err, result){
    ...
    delete : function(id, callback){
        pool.query(`delete from ${TABLE_NAME} where id = ?`, id, (err,result) => {
            if(err)
            {
                ...
                pool.query(`delete from cutting_program_inventory where vou_id = ?`, id, (err, res) => {
                    callback(false, result)
                })
            }
        })
