...
CuttingProgramModel.prototype = {
    ...
    checkAndSaveOrUpdate: function (body, callback) {
        ...
        if (issetNotEmpty(body.id)) {
            ...
            // body.created_at = new Date();
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where cuttingprogram = ?`, [body.cuttingprogram], (err, count) => {
                if (err) {
                    callback(err)
                } else {
                    // console.log("DB Query Success")
                    if (count[0].c > 0) {
                        callback("CuttingProgram Name Already Found!")
                    } else {
                        DBCON.query(`insert into ${TABLE_NAME} set ?`, body, (err, result) => {
                            if (err) {
                                callback(err)
                            } else {
                                callback(false, result, "CuttingProgram Saved Successfully!")
                            }
                        })
                    }
                }
