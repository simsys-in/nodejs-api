...
Product_CategoryModel.prototype = {
    ...
    checkAndSaveOrUpdate: function (body, callback) {
        ...
        if (issetNotEmpty(body.id)) {
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where id != ? and product_category = ?`, [body.id, body.name], (err, count) => {
                if (err) {
                    callback(err)
                } else {
                    if (count[0].c > 0) {
            ...
            // body.created_at = new Date();
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where product_category = ?`, [body.name], (err, count) => {
                if (err) {
                    callback(err)
                } else {
