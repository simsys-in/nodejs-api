...
BankModel.prototype = {
    ...
    checkAndSaveOrUpdate: function (body, callback) {
        ...
        if (issetNotEmpty(body.id)) {
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where id != ? and bank = ?`, [body.id, body.bank], (err, count) => {
                if (err) {
                    callback(err)
                } else {
                    if (count[0].c > 0) {
                        callback("Bank Already Found!")
                    } else {
                        ...
                        DBCON.query(`update ${TABLE_NAME} set ? where id = ?`, [body, body.id], (err, result) => {
                            if (err) {
                                ...
                                callback(false, result, "Bank Updated Successfully")
                            }
                        })
                    }
                }
            ...
            // body.created_at = new Date();
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where bank = ?`, [body.bank], (err, count) => {
                if (err) {
                    callback(err)
                } else {
                    // console.log("DB Query Success")
                    if (count[0].c > 0) {
                        callback("Bank Name Already Found!")
                    } else {
                        DBCON.query(`insert into ${TABLE_NAME} set ?`, body, (err, result) => {
                            if (err) {
                                callback(err)
                            } else {
                                callback(false, result, "Bank Saved Successfully!")
                            }
                        })
                    }
                }
