...
FabricInwardModel.prototype = {
    ...
    getAll : function(callback){
        pool.query(`select ${TABLE_NAME}.id,${TABLE_NAME}.vouno , DATE_FORMAT(${TABLE_NAME}.vou_date, '%d-%m-%Y') as vou_date , ledger.ledger from ${TABLE_NAME} left join ledger on ledger.id=fabric_inward.ledger_id order by ${TABLE_NAME}.id desc`, function(err, result){
                        ...
                        DBCON.query(GET_COLOR_DETAILS_QUERY, (err, color_details) => {
                            if (err) {
                                ...
                                //         jobwork_outward_details.accessories = accessories;
                                        ...
                                        const GET_LEDGER_DETAILS = `select ledger.ledger, ledger.delivery_address, ledger.mobile, ledger.phone, ledger.gstno from fabric_inward left join ledger on fabric_inward.ledger_id = ledger.id where fabric_inward.id = ${id}`;
