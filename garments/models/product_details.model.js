...
ProductDetailsModel.prototype = {
    ...
    checkAndSaveOrUpdate: function (body, callback) {
        ...
        if (issetNotEmpty(body.id)) {
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where id != ? and product_id = ? and size_id = ?`, [body.id, body.product_id, body.size_id], (err, count) => {
                if (err) {
                    callback(err)
                } else {
                    if (count[0].c > 0) {
            ...
            // body.created_at = new Date();
            DBCON.query(`select count(id) as c from ${TABLE_NAME} where product_id = ? and size_id = ?`, [body.product_id,body.size_id], (err, count) => {
                if (err) {
                    callback(err)
                } else {
